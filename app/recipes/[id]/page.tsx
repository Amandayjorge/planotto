...
export default function RecipeDetailPage() {
  ...
  const [translationLockReason, setTranslationLockReason] = useState<"guest" | "pro" | null>(null);
  ...
  useEffect(() => {
    if (canUseAiTranslation) {
      setTranslationLockReason((prev) => (prev === "pro" ? null : prev));
    }
  }, [canUseAiTranslation]);
  ...
  const createTranslationDraft = async (
    ...
    if (!recipe) return;
    if (!currentUserId) {
      setTranslationLockReason("guest");
      setTranslationNotice(t("recipes.detail.translation.guestLocked"));
      return;
    }
    if (!canUseAiTranslation) {
      setTranslationLockReason("pro");
      setTranslationNotice(t("recipes.detail.translation.proLocked"));
      return;
    }
    ...
    try {
      ...
      setTranslationLockReason(null);
      let draft = fallbackDraft;
      let notice = t("recipes.detail.translation.createdReview");
    ...
    <div style={{ padding: "20px", maxWidth: "900px", margin: "0 auto" }}>
      ...
      <div className="card" style={{ marginBottom: "14px", padding: "10px 12px" }}>
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", alignItems: "center" }}>
          <button
            type="button"
            className={`btn ${canUseAiTranslation && currentUserId ? "btn-primary" : ""}`.trim()}
            onClick={() => {
          ...
          >
            {isCreatingTranslation
              ...
              : !currentUserId
                ? t("recipes.detail.translation.guestOnlyCta")
                : canUseAiTranslation
                ? t("recipes.detail.translation.translateCta")
                : t("recipes.detail.translation.proOnlyCta")}
          </button>
        </div>
        {translationLockReason ? (
          <div style={{ marginTop: "8px", display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
            <span className="muted">
              🔒 {translationLockReason === "guest" ? t("recipes.detail.translation.guestLocked") : t("subscription.availableInPro")}
            </span>
            <button type="button" className="btn btn-primary" onClick={() => router.push("/auth")}>
              {translationLockReason === "guest" ? t("recipes.guestReminder.createAccount") : t("subscription.goToPro")}
            </button>
          </div>
        ) : null}
        {translationNotice ? (
